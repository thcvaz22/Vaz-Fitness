...
export default async function remapHandler(req,res){
  ...
  try{
    ...
    if(action==='reset_overrides'&&method==='POST'){
      const auth=await requireApprovedAthlete(req);if(!auth)return sendError(res,401,'Sessão inválida.','unauthorized');if(!auth.approved)return sendError(res,423,'Aluno ainda não está liberado.','athlete_not_approved');
      const state=await getState(auth.sql,auth.user.id),now=new Date().toISOString();
      const requests=safeArray(state.remapRequests,MAX_REQUESTS).map(item=>item?.status==='pending'?{...item,status:'superseded',decidedAt:now,decisionNote:'Substituído por uma nova configuração de dias de descanso.'}:item);
      await saveExperienceFields(auth.sql,auth.user.id,{requests,overrides:{}});
      await audit(auth.sql,auth.user.id,auth.user.id,'weekly_remap_reset',{reason:clean(req.body?.reason,120)||'rest_days_changed'});
      return res.status(200).json({ok:true,weekOverrides:{},requests:requests.map(publicRequest)});
    }
