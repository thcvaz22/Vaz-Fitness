  ...
  try{
    if(action==='summary'&&method==='GET'){
      const rows=await sql`SELECT u.id,u.public_code,u.name,u.email,u.username,u.account_status,u.created_at,s.monthly_amount,s.next_due_date,s.payment_status,s.notes AS subscription_notes,
        (SELECT count(*)::int FROM vf_athlete_access aa WHERE aa.personal_id=u.id) AS athlete_count,
        (SELECT count(*)::int FROM vf_athlete_access aa WHERE aa.personal_id=u.id AND aa.status='approved') AS active_athletes
