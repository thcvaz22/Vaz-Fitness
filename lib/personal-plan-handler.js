...
function money(v){const n=Number(v);return Number.isFinite(n)&&n>=0&&n<=999999?n:null}
...
async function getAssignment(sql,personalId){
  const rows=await sql`SELECT s.personal_id,s.plan_code,s.monthly_amount,p.name AS plan_name,p.student_limit,
    (SELECT count(*)::int FROM vf_athlete_access a WHERE a.personal_id=s.personal_id AND a.athlete_id<>a.personal_id) AS student_count
  ...
  try{
    if(action==='summary'&&method==='GET'){
      ...
      const rows=await sql`SELECT u.id AS personal_id,s.plan_code,s.monthly_amount,p.name AS plan_name,p.student_limit,
        (SELECT count(*)::int FROM vf_athlete_access a WHERE a.personal_id=u.id AND a.athlete_id<>a.personal_id) AS student_count
    ...
    if(action==='update_pricing'&&method==='POST'){
      const planCode=clean(req.body?.planCode,40),monthlyPrice=money(req.body?.monthlyPrice),activationFee=money(req.body?.activationFee??0),applyToExisting=req.body?.applyToExisting===true;
      if(monthlyPrice==null||activationFee==null)return sendError(res,400,'Informe valores válidos para o plano.','invalid_price');
      const rows=await sql`SELECT code,name,student_limit,monthly_price,activation_fee FROM vf_personal_plans WHERE code=${planCode} LIMIT 1`;
      const before=rows[0];if(!before)return sendError(res,404,'Plano não encontrado.','plan_not_found');
      await sql`UPDATE vf_personal_plans SET monthly_price=${monthlyPrice},activation_fee=${activationFee},updated_at=now() WHERE code=${planCode}`;
      let updatedSubscriptions=0;
      if(applyToExisting){const changed=await sql`UPDATE vf_personal_subscriptions SET monthly_amount=${monthlyPrice},updated_at=now() WHERE plan_code=${planCode} RETURNING personal_id`;updatedSubscriptions=changed.length}
      await audit(sql,user.id,user.id,'personal_plan_pricing_updated',{planCode,oldMonthlyPrice:Number(before.monthly_price),newMonthlyPrice:monthlyPrice,oldActivationFee:Number(before.activation_fee),newActivationFee:activationFee,applyToExisting,updatedSubscriptions});
      return res.status(200).json({ok:true,plan:{...mapPlan(before),monthlyPrice,activationFee},applyToExisting,updatedSubscriptions});
    }
