...
import { requireApprovedAthlete } from './session-auth.js';
...
function validUserId(value=''){return /^[A-Za-z0-9_-]{8,180}$/.test(String(value))}
function stateSecret(){return process.env.STRAVA_STATE_SECRET||process.env.STRAVA_CLIENT_SECRET||''}
export function makeState(installationId,returnTo='/',userId=null){
  requireStravaConfig();
  if(!validInstallationId(installationId))throw new Error('installationId inválido.');
  const payloadData={installationId,returnTo,ts:Date.now()};
  if(userId&&validUserId(userId))payloadData.userId=String(userId);
...
export function readState(state=''){
  ...
  if(data.userId&&!validUserId(data.userId))throw new Error('Usuário OAuth inválido.');
  return data;
}
export async function saveConnection(installationId,token,userId=null){
  const db=sql();
  const athlete=token.athlete||{};
  const name=[athlete.firstname,athlete.lastname].filter(Boolean).join(' ')||null;
  await db`INSERT INTO strava_connections (installation_id,athlete_id,athlete_name,access_token,refresh_token,expires_at,scope,auto_sync,user_id,updated_at)
    VALUES (${installationId},${athlete.id||null},${name},${token.access_token},${token.refresh_token},${Number(token.expires_at)},${token.scope||''},true,${userId||null},now())
    ON CONFLICT (installation_id) DO UPDATE SET athlete_id=EXCLUDED.athlete_id, athlete_name=EXCLUDED.athlete_name, access_token=EXCLUDED.access_token, refresh_token=EXCLUDED.refresh_token, expires_at=EXCLUDED.expires_at, scope=EXCLUDED.scope, auto_sync=true, user_id=COALESCE(EXCLUDED.user_id,strava_connections.user_id), updated_at=now()`;
}
export async function getConnection(installationId){
  if(!validInstallationId(installationId))return null;
  const db=sql();
  const rows=await db`SELECT installation_id,athlete_id,athlete_name,access_token,refresh_token,expires_at,scope,auto_sync,user_id FROM strava_connections WHERE installation_id=${installationId} LIMIT 1`;
  return rows[0]||null;
}
export async function getConnectionForUser(installationId,userId,{claimLegacy=true}={}){
  const conn=await getConnection(installationId);
  if(!conn)return null;
  if(conn.user_id&&conn.user_id!==userId)return null;
  if(!conn.user_id&&claimLegacy){
    const db=sql();
    const rows=await db`UPDATE strava_connections SET user_id=${userId},updated_at=now() WHERE installation_id=${installationId} AND user_id IS NULL RETURNING installation_id`;
    if(!rows.length)return null;
    conn.user_id=userId;
  }
  return conn;
}
export async function getConnectionByAthleteId(athleteId){
  const id=Number(athleteId);if(!Number.isFinite(id))return null;
  const db=sql();
  const rows=await db`SELECT installation_id,athlete_id,athlete_name,access_token,refresh_token,expires_at,scope,auto_sync,user_id FROM strava_connections WHERE athlete_id=${id} LIMIT 1`;
  return rows[0]||null;
}
export async function authenticateStravaAthlete(req){
  const auth=await requireApprovedAthlete(req);
  if(!auth||!auth.approved)return null;
  return auth;
}
...
export function stravaRateInfo(headers){
  const readLimit=headers?.get?.('X-ReadRateLimit-Limit')||null;
  const readUsage=headers?.get?.('X-ReadRateLimit-Usage')||null;
  const overallLimit=headers?.get?.('X-RateLimit-Limit')||null;
  const overallUsage=headers?.get?.('X-RateLimit-Usage')||null;
  return {readLimit,readUsage,overallLimit,overallUsage};
}
...
export function makeGpx(session){
  ...
  return `<?xml version="1.0" encoding="UTF-8"?><gpx version="1.1" creator="Vaz Fitness" xmlns="http://www.topografix.com/GPX/1/1"><metadata><name>${escXml(session.name||'Corrida Vaz Fitness')}</name></metadata><trk><name>${escXml(session.name||'Corrida Vaz Fitness')}</name><type>running</type><trkseg>${trk}</trkseg></gpx>`;
}
