...
function finishWorkout(){
  ...
  const session={id:Date.now(),date:new Date().toISOString(),name:item.name,duration,volume:Math.round(volume),exercises,muscleScore};
  state.sessions.push(session); item.status='done'; state.current=null; state.score=Math.min(99,state.score+1); save();
