...
(function installCycleCalendarV7(){
  ...
  function renderCycleCalendar(){
    ...
    for(let d=1;d<=count;d++){
      ...
      const weekday=new Date(y,m,d,12).getDay(),restDay=inside&&(state.profile?.restDays||[]).map(Number).includes(weekday),anticipatedRest=inside&&!list.length&&anticipatedList().some(x=>x?.originalDate===dk);
      const emptyLabel=anticipatedRest?'Descanso • treino antecipado':restDay?'Descanso':'Sem treino';
      cells.push(`<button class="cycle-day ${tone?`tone-${tone}`:''} ${today?'today':''} ${inside?'in-cycle':'out-cycle'}" data-cycle-date="${dk}" ${!list.length?`aria-label="${safe(emptyLabel)}"`:''}><span class="cycle-number">${d}</span>${list.length?`<div class="cycle-marks">${list.slice(0,4).map(e=>`<span title="${safe(e.name)}">${shortLabel(e)}</span>`).join('')}</div><small>${list.length===1?safe(list[0].name):`${list.length} atividades`}</small>`:`<small class="rest-label">${inside?emptyLabel:''}</small>`}</button>`);
    }
