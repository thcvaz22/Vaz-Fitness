  ...
  try{
    if(req.method==='GET'&&action==='summary'){
      ...
      const alerts=[];for(const c of clients){
        ...
        if(c.attention?.readiness)alerts.push({kind:'recovery',severity:'warning',priority:78,athleteId:c.id,name:c.name,message:`Prontidão recente baixa: ${c.attention.readiness.score}%.`,readiness:c.attention.readiness,actionTab:'summary'});
