...
import { EXERCISE_CATALOG, EXERCISE_CATALOG_VERSION } from '../lib/exercise-catalog-v4.js';
...
async function customExercises(sql,personalId){
  try{
    const rows=await sql`SELECT id,name,muscle,secondary,equipment,default_sets,default_reps,default_rest,instructions,video_url,image_url FROM vf_personal_exercises WHERE personal_id=${personalId} AND is_archived=false ORDER BY name`;
    return rows.map(r=>({id:r.id,name:r.name,muscle:r.muscle,secondary:Array.isArray(r.secondary)?r.secondary:[],equipment:r.equipment||'Personal',sets:Number(r.default_sets)||3,reps:r.default_reps||'8-12',rest:Number(r.default_rest)||60,instructions:r.instructions||'',videoUrl:r.video_url||null,imageUrl:r.image_url||null,mediaQuery:r.name,source:'personal'}));
  }catch{return []}
}
...
export default async function handler(req,res){
  ...
  try{
    ...
    if(action==='suggest_plan'&&method==='POST'){
      ...
      const plan=canonicalizePlan(data.plan,available);if(!plan.length)return sendError(res,502,'AION não conseguiu montar o plano agora.','empty_ai_plan');
