...
async function issueSession(sql,userId){const token=crypto.randomBytes(32).toString('base64url'),expires=new Date(Date.now()+SESSION_DAYS*86400000).toISOString();await sql`INSERT INTO vf_sessions(token_hash,user_id,expires_at) VALUES (${tokenHash(token)},${userId},${expires})`;return token}
async function auth(req){const h=String(req.headers?.authorization||''),token=h.startsWith('Bearer ')?h.slice(7).trim():'';if(!token)return null;const sql=db();const rows=await sql`SELECT u.id,u.public_code,u.role,u.email,u.name,u.username,u.account_status,u.must_change_password,s.token_hash FROM vf_sessions s JOIN vf_users u ON u.id=s.user_id WHERE s.token_hash=${tokenHash(token)} AND s.expires_at>now() LIMIT 1`;return rows[0]?{sql,user:rows[0],tokenHash:rows[0].token_hash}:null}
async function maybeBootstrapAdmin(sql,identifier,password){
  ...
  const existing=await sql`SELECT * FROM vf_users WHERE role='admin' LIMIT 1`;if(existing.length)return existing[0];
  if(!safeEqualHex(passwordHash(password,BOOTSTRAP_SALT),BOOTSTRAP_HASH))return null;
...
export default async function handler(req,res){
  ...
  try{
    ...
    if(action==='login'&&method==='POST'){
      ...
      if(!safeEqualHex(passwordHash(password,row.password_salt),row.password_hash))return sendError(res,401,'Usuário/e-mail ou senha inválidos.','invalid_credentials');
    ...
    if(action==='change_password'&&method==='POST'){
      ...
      const rows=await a.sql`SELECT password_salt,password_hash FROM vf_users WHERE id=${a.user.id} LIMIT 1`,r=rows[0];if(!r||!safeEqualHex(passwordHash(current,r.password_salt),r.password_hash))return sendError(res,401,'Senha atual incorreta.','invalid_current_password');
      const salt=crypto.randomBytes(16).toString('hex'),hash=passwordHash(next,salt);await a.sql`UPDATE vf_users SET password_salt=${salt},password_hash=${hash},must_change_password=false,updated_at=now() WHERE id=${a.user.id}`;await a.sql`DELETE FROM vf_sessions WHERE user_id=${a.user.id}`;const token=await issueSession(a.sql,a.user.id);return res.status(200).json({ok:true,token,mustChangePassword:false});
    }
