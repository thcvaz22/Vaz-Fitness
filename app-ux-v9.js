...
(function installUxV9(){
  ...
  function stripProfileExtras(html){
    ...
    const achTab=root.querySelector('[data-exp-tab="achievements"]');if(achTab)achTab.textContent='Conquistas';
  ...
  render=function(){
    ...
    const custom=['calendar','goals','metrics'];
    if(custom.includes(activeView)){
      const v=document.getElementById('view');
      if(v){v.innerHTML=activeView==='calendar'?renderCalendarPage():activeView==='goals'?renderGoalsPage():renderMetricsPage();bindDynamic();updateNav();}
      return;
    }
