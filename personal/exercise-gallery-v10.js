...
(()=>{
  ...
  renderExercise=function(e,di,ei){
    ...
    const guide=!!(base.instructions||base.imageUrl||base.videoUrl||base.mediaQuery||base.source!=='personal');
  ...
  function mediaHtml(e,images,instructions,tips,video){return `<div class="vp-media-grid">${images.length?`<div class="vp-media-images">${images.slice(0,2).map((src,i)=>`<figure><img src="${safe(src)}" alt="${i?'Posição final':'Posição inicial'} de ${safe(e.name)}"><figcaption>${i?'Posição final':'Posição inicial'}</figcaption></figure>`).join('')}</div>`:'<div class="vp-media-noimage">Sem imagem cadastrada.</div>'}<div class="vp-media-copy"><h3>Como executar</h3><ol>${instructions.slice(0,7).map(x=>`<li>${safe(x)}</li>`).join('')}</ol>${tips?.length?`<h3>Pontos de atenção</h3><ul>${tips.slice(0,5).map(x=>`<li>${safe(x)}</li>`).join('')}</ul>`:''}${video?`<a class="vp-btn dark" href="${safe(video)}" target="_blank" rel="noopener">▶ Abrir vídeo demonstrativo</a>`:''}</div></div>`}
