...
(function installExperienceV6(){
  ...
  function medalFor(c){
    return {id:`medal-${c.id}`,challengeId:c.id,title:c.title,description:c.description,difficulty:c.difficulty||1,kind:c.medalKind||c.metric||'star',earnedAt:new Date().toISOString(),month:monthKey()};
  }
  function evaluateChallenges(){
    ensureMonthlyChallenges();
    [...state.monthlyChallenges,...state.customGameGoals].forEach(c=>{
      if(c.completedAt)return;const value=challengeProgress(c);if(c.metric!=='manual'&&value>=Number(c.target||0)){c.completedAt=new Date().toISOString();if(!state.medalHall.some(m=>m.challengeId===c.id))state.medalHall.push(medalFor(c));}
    });
  ...
  function syncGoalContext(){
    state.goals.gamification={month:monthKey(),active:[...state.monthlyChallenges,...state.customGameGoals].filter(x=>!x.completedAt).slice(0,12).map(x=>({title:x.title,metric:x.metric,target:x.target,current:challengeProgress(x),difficulty:x.difficulty}))};
  }
  ...
  function feedbackLabels(v,type){
    const n=Number(v);if(type==='fatigue')return n<=3?'Pouco cansado':n<=6?'Cansaço moderado':n<=8?'Bem cansado':'Muito cansado';
  ...
  function feedbackCard(s,{run=false}={}){
    ...
    return `<div class="card vf-session-feedback" data-feedback-session="${s.id}" data-feedback-run="${run?'1':'0'}"><div class="card-head"><div><span class="eyebrow">COMO FOI O TREINO?</span><h3>Seu feedback melhora os próximos ajustes.</h3><p>A AION e seu personal usarão essa percepção junto com os dados do treino.</p></div></div>${[['intensity','Intensidade do treino',f.intensity],['fatigue','Quanto cansado você ficou',f.fatigue],['effort','Quanto esforço você fez',f.effort]].map(([k,l,v])=>`<label class="vf-feedback-range"><div><strong>${l}</strong><span data-feedback-label="${k}">${feedbackLabels(v,k)}</span></div><input type="range" min="1" max="10" value="${v}" data-feedback-range="${k}"><div class="vf-range-scale"><small>1</small><b>${v}/10</b><small>10</small></div></label>`).join('')}<label class="vf-feedback-notes"><strong>Observações</strong><textarea rows="3" data-feedback-notes placeholder="Dor, exercício desconfortável, energia, algo que queira contar ao personal…">${safe(f.notes||'')}</textarea></label><div class="dialog-actions"><button class="btn ghost" data-feedback-skip>Agora não</button><button class="btn primary" data-feedback-save>Salvar feedback e concluir</button></div></div>`;
  }
  function bindFeedback(root,s,closeFn){
    root.querySelectorAll('[data-feedback-range]').forEach(inp=>inp.addEventListener('input',()=>{root.querySelector(`[data-feedback-label="${inp.dataset.feedbackRange}"]`).textContent=feedbackLabels(inp.value,inp.dataset.feedbackRange);root.querySelector(`[data-feedback-range="${inp.dataset.feedbackRange}"]`)?.closest('.vf-feedback-range')?.querySelector('.vf-range-scale b')&&(inp.closest('.vf-feedback-range').querySelector('.vf-range-scale b').textContent=`${inp.value}/10`)}));
  ...
  showSummary=function(s){
    baseShowSummary(s);const box=document.getElementById('summaryContent');if(!box)return;box.querySelector('.dialog-actions')?.remove();box.insertAdjacentHTML('beforeend',feedbackCard(s));const card=box.querySelector('.vf-session-feedback');bindFeedback(card,s,()=>document.getElementById('summaryDialog').close());
  };
  if(typeof startRun==='function'){
    const baseStartRun=startRun;
    startRun=async function(item){const before=(state.runSessions||[]).length;const out=await baseStartRun(item);if((state.runSessions||[]).length>before){const s=state.runSessions.at(-1);const d=document.getElementById('summaryDialog'),box=document.getElementById('summaryContent');if(d&&box){box.innerHTML=`<div class="summary-wrap"><div class="summary-hero"><span class="eyebrow">CORRIDA CONCLUÍDA</span><h2>${safe(s.name||'Corrida')}</h2><p style="color:#bbb">${safe(s.pace||'')} ${s.distance?`• ${Number(s.distance).toFixed(1)} km`:''}</p></div>${feedbackCard(s,{run:true})}</div>`;d.showModal();bindFeedback(box.querySelector('.vf-session-feedback'),s,()=>d.close());}}return out;};
  }
  ...
  function healthModal(){
    const d=document.createElement('dialog');d.className='dialog vf-health-dialog';d.innerHTML=`<form method="dialog" id="vfHealthForm"><span class="eyebrow">NOVO REGISTRO</span><h2>Lesão ou limitação</h2><div class="field-row"><label>Tipo<select name="type"><option value="injury">Lesão</option><option value="limitation">Limitação</option></select></label><label>Região / condição<input name="area" required maxlength="80" placeholder="Ex.: joelho direito"></label></div><div class="field-row"><label>Status<select name="status"><option value="active">Ativa</option><option value="recovering">Em recuperação</option><option value="resolved">Resolvida</option></select></label><label>Nível de atenção (1–5)<input name="severity" type="range" min="1" max="5" value="3"></label></div><label>Movimentos / situações a evitar ou limitar<textarea name="avoid" rows="2" maxlength="500" placeholder="Ex.: impacto, agachamento profundo, corrida em descida…"></textarea></label><label>Orientação de fisioterapeuta/médico/profissional, se houver<textarea name="guidance" rows="2" maxlength="500"></textarea></label><label>Observações<textarea name="notes" rows="3" maxlength="800"></textarea></label><div class="dialog-actions"><button type="button" class="btn ghost" data-health-cancel>Cancelar</button><button class="btn primary" type="submit">Salvar registro</button></div></form>`;document.body.appendChild(d);d.showModal();d.querySelector('[data-health-cancel]').onclick=()=>{d.close();d.remove()};d.querySelector('form').onsubmit=e=>{e.preventDefault();const f=new FormData(e.currentTarget);state.healthRecords.push({id:uid('health'),type:f.get('type'),area:String(f.get('area')||'').trim(),status:f.get('status'),severity:Number(f.get('severity'))||1,avoid:String(f.get('avoid')||'').trim(),guidance:String(f.get('guidance')||'').trim(),notes:String(f.get('notes')||'').trim(),createdAt:new Date().toISOString()});state.healthRecords=state.healthRecords.slice(-40);save();d.close();d.remove();render();toast('Registro salvo e incluído no contexto da AION.')};
  }

  function medalSvg(kind='star',difficulty=1){
    const d=clamp(difficulty,1,4),symbol={run:'➤',strength:'◆',calendar:'✓',mind:'◎',flame:'✦',star:'★',manual:'★'}[kind]||'★';
    const points=d>=4?'50,4 61,32 92,24 75,50 94,74 63,68 50,96 37,68 6,74 25,50 8,24 39,32':'50,8 78,22 92,50 78,78 50,92 22,78 8,50 22,22';
    return `<svg viewBox="0 0 100 118" aria-hidden="true"><path d="M31 75 L20 116 49 101 50 75Z" class="rib a"/><path d="M69 75 L80 116 51 101 50 75Z" class="rib b"/><polygon points="${points}" class="medal-shape"/><circle cx="50" cy="50" r="${d>=3?30:25}" class="medal-ring"/><text x="50" y="59" text-anchor="middle" class="medal-symbol">${symbol}</text>${d>=3?'<circle cx="50" cy="50" r="37" class="medal-dots"/>':''}</svg>`;
  }
  function goalCard(c,custom=false){const current=challengeProgress(c),target=Number(c.target)||1,pct=Math.min(100,Math.round(current/target*100)),done=!!c.completedAt;return `<article class="vf-goal-card ${done?'done':''}"><div class="vf-goal-medal tier-${c.difficulty||1} ${done?'earned':'locked'}">${medalSvg(c.medalKind||c.metric,c.difficulty||1)}</div><div class="vf-goal-copy"><span>${difficultyName(c.difficulty||1)}</span><h3>${safe(c.title)}</h3><p>${safe(c.description||metricLabel(c.metric))}</p><div class="vf-goal-progress"><i style="width:${pct}%"></i></div><small>${c.metric==='manual'?(done?'Concluída':'Marque quando concluir'):`${Number(current).toFixed(c.metric==='runKm'?1:0)}${targetUnit(c.metric)} / ${target}${targetUnit(c.metric)} • ${pct}%`}</small>${custom&&c.metric==='manual'&&!done?`<button class="btn primary compact" data-goal-complete="${c.id}">Concluir meta</button>`:''}</div></article>`}
  function achievementsView(){
    evaluateChallenges();const earned=state.medalHall.slice().reverse();
    return `<section class="vf-experience-page"><div class="vf-game-hero"><div><span class="eyebrow">METAS & CONQUISTAS</span><h1>Seu mês virou uma jornada.</h1><p>Cumpra desafios, crie metas próprias e monte seu Hall de Medalhas.</p></div><div class="vf-game-level"><small>MEDALHAS</small><strong>${earned.length}</strong><span>${earned.filter(x=>x.difficulty>=3).length} raras</span></div></div><div class="card"><div class="card-head"><div><h2>Desafios de ${new Date().toLocaleDateString('pt-BR',{month:'long'})}</h2><p>O sistema cria desafios compatíveis com sua modalidade e rotina.</p></div><button class="btn primary compact" data-goal-add>+ Criar minha meta</button></div><div class="vf-goal-grid">${state.monthlyChallenges.map(c=>goalCard(c)).join('')}${state.customGameGoals.filter(x=>!x.completedAt).map(c=>goalCard(c,true)).join('')}</div></div><div class="card vf-medal-hall" style="margin-top:18px"><div class="card-head"><div><span class="eyebrow">HALL DE MEDALHAS</span><h2>Sua coleção</h2><p>Desafios mais difíceis geram medalhas maiores e mais detalhadas.</p></div></div>${earned.length?`<div class="vf-medal-grid">${earned.map(m=>`<article class="vf-medal tier-${m.difficulty}">${medalSvg(m.kind,m.difficulty)}<strong>${safe(m.title)}</strong><small>${difficultyName(m.difficulty)} • ${new Date(m.earnedAt).toLocaleDateString('pt-BR')}</small></article>`).join('')}</div>`:'<div class="vf-empty-hall"><div class="vf-goal-medal tier-3 locked">${medalSvg('star',3)}</div><strong>Sua primeira medalha está esperando.</strong><span>Complete um desafio do mês para iniciar sua coleção.</span></div>`}</div></section>`;
  }
  function goalModal(){
    const d=document.createElement('dialog');d.className='dialog vf-health-dialog';d.innerHTML=`<form id="vfGoalForm"><span class="eyebrow">NOVA META</span><h2>Crie seu próprio desafio</h2><label>Nome da meta<input name="title" required maxlength="70" placeholder="Ex.: completar 12 treinos"></label><div class="field-row"><label>Medir por<select name="metric"><option value="sessions">Treinos concluídos</option><option value="strengthSessions">Treinos de força</option><option value="runSessions">Corridas</option><option value="runKm">Km de corrida</option><option value="activeMinutes">Minutos ativos</option><option value="feedbacks">Feedbacks pós-treino</option><option value="manual">Meta livre / manual</option></select></label><label>Meta<input name="target" type="number" min="1" step="1" value="8"></label></div><label>Dificuldade<select name="difficulty"><option value="1">Bronze</option><option value="2" selected>Prata</option><option value="3">Ouro</option><option value="4">Lendária</option></select></label><label>Descrição<input name="description" maxlength="140" placeholder="O que você quer conquistar?"></label><div class="dialog-actions"><button type="button" class="btn ghost" data-goal-cancel>Cancelar</button><button class="btn primary">Criar desafio</button></div></form>`;document.body.appendChild(d);d.showModal();d.querySelector('[data-goal-cancel]').onclick=()=>{d.close();d.remove()};d.querySelector('form').onsubmit=e=>{e.preventDefault();const f=new FormData(e.currentTarget),metric=f.get('metric');state.customGameGoals.push({id:uid('goal'),title:String(f.get('title')||'').trim(),description:String(f.get('description')||'').trim(),metric,target:metric==='manual'?1:Math.max(1,Number(f.get('target'))||1),difficulty:clamp(f.get('difficulty'),1,4),medalKind:metric==='runKm'||metric==='runSessions'?'run':metric==='strengthSessions'?'strength':metric==='activeMinutes'?'flame':metric==='feedbacks'?'mind':'manual',createdAt:new Date().toISOString()});save();d.close();d.remove();render();toast('Meta criada. Boa jornada!')};
  }
  ...
  buildAionContext=function(){const ctx=baseContext();syncExperience();ctx.healthContext=state.profile.healthContext||{};ctx.recentWorkoutFeedback=(state.sessions||[]).slice(-8).map(s=>({date:s.date,name:s.name,feedback:s.sessionFeedback||null})).filter(x=>x.feedback);ctx.gameGoals=state.goals.gamification||{};return ctx;};
